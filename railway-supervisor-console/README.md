...
JavaFX desktop demo that mimics an Indian Railways PRS/UTS supervisor terminal for a **QR Fare Repeater Display System** proof-of-concept. It builds pipe-delimited ticket packets and sends them over **serial COM3** by default (9600 8N1, UTF-8 text + newline) via **jSerialComm**, paired with the QFRDS controller on **COM4** (com0com). If the port is unavailable, the app runs in **mock mode** and logs packets only.

## Requirements

- JDK **17**
- Maven **3.8+**
- Windows with **COM3** virtual pair to controller **COM4** (or adjust `SerialService.DEFAULT_PORT_NAME`)
...
| `SerialService` | jSerialComm: COM3 (default), 9600 8N1, UTF-8 + LF |
